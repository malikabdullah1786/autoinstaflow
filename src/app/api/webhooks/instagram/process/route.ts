...
// POST: Receives individual queued events forwarded securely from Upstash QStash
export async function POST(req: Request) {
  try {
    const rawBody = await req.text();
    const signature = req.headers.get('upstash-signature');

    // Secure the receiver using QStash keys to prevent unauthorized calls
    ...
    const { entry, instagramAccountId } = payload;
    ...
    if (entry.changes) {
      for (const change of entry.changes) {
        if (change.field === 'comments') {
          ...
          console.log(`[Queue Process] Processing comment: "${commentText}" from @${senderUsername} on post ${postId}`);
          ...
          // Check 24-hour deduplication - 10s throttle for safety/testing
    ...
    if (entry.messaging) {
      for (const messageEvent of entry.messaging) {
        if (messageEvent.message && !messageEvent.message.is_echo) {
          ...
          console.log(`[Queue Process] Processing DM from ${senderId}: "${messageText}"`);
    ...
    console.error('Queue Process handler exception:', e);
